...
class ExperimentalLoraGPSPage extends StatefulWidget {
  ExperimentalLoraGPSPage({Key key}) : super(key: key);

  @override
  _ExperimentalLoraGPSPageState createState() => _ExperimentalLoraGPSPageState();
}

class _ExperimentalLoraGPSPageState extends State<ExperimentalLoraGPSPage> {

  Future<List<Coordinate>> last_coordinate;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<Coordinate>>(
        future: last_coordinate,
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            List<Coordinate> data = snapshot.data;
  ...
  Widget _stackcreation(List<Coordinate> data){
    return Stack(children: <Widget>[
      _crearMapa(data),
    ],);
  } 
  
  Widget _crearMapa(List<Coordinate> data) {
    return Container(
      height: 800.0,
      width: 420.0,
      ...
      child: FlutterMap(
              options: new MapOptions(
                zoom: 16.0,
                center: LatLng(42.4654015,-2.4347036666666666),
              ),
              layers: [
  ...
  Future<List<Coordinate>> fetchPost() async {
      final response =
      await http.get('http://83.32.158.101:3002/lora_gps');

      if (response.statusCode == 200) {
        // Si el servidor devuelve una repuesta OK, parseamos el JSON
        List jsonResponse = json.decode(response.body);
        return jsonResponse.map((convocatoriaitem) => new Coordinate.fromJson(convocatoriaitem)).toList();
      } else {
  ...
  List<Marker> _markerlist(List<Coordinate> data) {
      List<Marker> result = data.map((coordenada) {
        // Preprocess Data that comes from API Rest
        String latitud = coordenada.latitud;
        String longitud  = coordenada.longitud;
        print(coordenada.id);
        //String fechahora = coordenada.fechahora;
        ...
        double finallatitud = 42.0;
        double finallongitud = -2.0;

        if( latitudList.length > 1 && coordenada.id != 17459 && coordenada.id != 17432) {
      ...
      if (longitudList.length > 1 && coordenada.id != 17459 && coordenada.id != 17432) {
      ...
      return new Marker(
                      ...
                      builder: (ctx) =>
                      new Container(
                        child: new Icon(Icons.gps_fixed, size: 10.0,color: Colors.redAccent,),
                      ),
                    );
